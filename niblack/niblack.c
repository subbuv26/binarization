...
#define BLACK 0
#define WHITE 255
#define kCONSTANT -0.5

void niblack(uchar *, uchar *, int , int , int );
...
int main(int argc, char **argv) {
   ...
   if(argc != 3) {
      printf("Usage: ./niblackBinarization <input_image> <output_image>\n");
      exit(-1);
   }
   ...
   for(i = 0; i < height; i += Wsize) {
      ...
      for(j = 0; j < width; j += Wsize) {
         ...
         niblack(Idata + i*step + j, Odata + i*step + j, step, wh, ww);
      }
   }
...
void niblack(unsigned char *Idat, unsigned char *Odat, int step, int h, int w)
{
   ...
   Threshold = Mean + kCONSTANT * StdDev;
   for(i = 0; i < h; i++) {
      for(j = 0; j < w; j++) {
         pixel = (int) Idat[i*step + j];
         if(pixel < Threshold)
